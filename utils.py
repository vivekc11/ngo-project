...
# Add project root to sys.path to resolve imports from other directories
project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
if project_root not in sys.path:
    sys.path.insert(0, project_root)

# Now, import config.py
from config import MAX_TEXT_LENGTH
...
def extract_main_text_from_html(html_content: str) -> str:
    """
    Extracts main, readable text from a raw HTML string using trafilatura.
    Removes boilerplate content like menus, footers, and ads.
    """
    if not html_content:
        return ""
    
    # Use trafilatura to get clean content
    extracted_text = trafilatura.extract(html_content, include_comments=False, no_fallback=True)
    
    # Fallback to a simpler lxml-based string extraction if trafilatura fails
    if not extracted_text:
        try:
            from lxml.html import fromstring
            tree = fromstring(html_content)
            extracted_text = ' '.join(tree.xpath('//text()')).strip()
        except Exception:
            extracted_text = ""
