# # populate_fake_grants.py

# import os # NEW: Added import for os module for path manipulation
# import sys
# from datetime import datetime, timedelta
# import random
# from typing import Dict, List # Corrected and added typing imports

# # Adjust this path if populate_fake_grants.py is not in the same directory as grants_db.py and utils.py
# # This line attempts to add the current script's directory to Python's search path,
# # which helps in locating sibling modules like grants_db and utils.
...
#     inserted_count: int = 0 # Added type hint
#     for grant_data in FAKE_GRANTS_DATA:
#         grant_data["link_hash"] = generate_link_hash(grant_data["link"])

#         grant_data["application_deadline"] = datetime.strptime(
#             grant_data["application_deadline"], "%Y-%m-%d"
#         ) if grant_data["application_deadline"] else None

#         if insert_grant(grant_data):
#             inserted_count += 1
#             print(f"  Inserted: {grant_data['title']} (SDGs: {', '.join(grant_data['sdg_tags'])})")
#         else:
#             print(f"  Skipped (already exists or error): {grant_data['title']}")
            
#     print(f"Finished. Successfully attempted to insert {len(FAKE_GRANTS_DATA)} grants.")
#     print(f"Total new grants inserted: {inserted_count}.")

# if __name__ == "__main__":
#     populate_database_with_fake_grants()